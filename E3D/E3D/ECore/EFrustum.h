...
namespace E3D
{
	...
	{
		EInt state;							//摄像机状态
		EInt sttribute;                     //摄像机属性
		CAMERA_TYPE camMode;				//

		EVector4D positio;                //摄像机在世界坐标系的位置
		...
		EMatrix44 mCamerToPerspective;				//相机坐标到透视坐标变换矩阵
		EMatrix44 mPerspectiveToScreen;				//透视坐标到屏幕坐标变换矩阵

		EFrustum(CAMERA_TYPE mode, const EVector4D &pos, const EVector4D &dir, const EVector4D &target, EFloat nearZ, EFloat forZ, EFloat ffov, EFloat viewportWidth, EFloat viewportHeight);
	};
}