...
namespace E3D
{
	const EString EBullet_Ball = "BallBullet.mesh";
	...
	EGameManager::EGameManager(ESceneManager * scene) :mScene(scene), mBlockSize(0.0f), mX(0), mZ(0), mTerrainMesh(NULL), mCurrentEnemyNumber(0), mMaxEnemyNumber(0), mVisibleEnemyNumber(3)
	{
	...
	{
		EString meshname = (owner->getBulletType() == BULLET_ROCKET ? Bullet_Rocket : EBullet_Ball);
		EBullet * bullet = new EBullet(owner, meshname, this);
		if (meshname == EBullet_Ball)
		{
		...
		while (in.good())
		{
			...
			if (info.find("Nodes") != -1)
			{
				...
				while (info.find("Node") != -1)
				{
					...
					//<Position X="-49.649979" Y="4247236" Z="-5.005510"/>
					in.getline(line, 256);
					info = Trim(line);
	...
	void EGameManager::playSound(SOUND_YYPE sound, EBool loop)
	{
		...
		switch (sound)
		{
		case E3D::SOUND_BACKGROUND:
			soundName = Background;
			break;
		case E3D::SOUND_FIRE:
			soundName = Fire;
			break;
		case E3D::SOUND_EXPLODE:
			soundName = Explode;
			break;
		};
