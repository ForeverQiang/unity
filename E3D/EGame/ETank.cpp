...
namespace E3D
{
	ETank::ETank(const EString &name, const EString& meshName, EGameManager* gameMgr) :mGameMgr(gameMgr), mScene(gameMgr->getSceneManager()), mSpeed(0.75f), mFireDir(0, 0, -1), mLastFire(0), mBulletType(BULLET_ROCKET), mMoveVec(0, 0, 0), mColOffset(3.5f), mTankType(TANK_PLAYER)
	{
		mMaxLive = 100;
		mCurrentLive = 0;
		mFireFrequency = 8;
	...
	EBool ETank::moveUp()
	{
		...
		//首先检测是否有障碍物阻止
		//从Tank的左侧和右侧发出两条射线
		if (mGameMgr->canGo(pos.x + mMoveVec.x + xOff, pos.z+mMoveVec.z + zOff) && mGameMgr->canGo(pos.x + mMoveVec.x -xOff,pos.z + mMoveVec.z - zOff))
		{
			move(mMoveVec);
			return true;
		}
		else         //有，那么坦克和摄像机偏移量重置为0
		{
