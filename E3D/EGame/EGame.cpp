...
namespace E3D
{
	...
	EBool EGame::keyRlease(EInt key)
	{
	...
	void EGame::showInfo()
	{
		...
		if (showHelp)
		{
			scene->drawString("1.W A S D 移动坦克", 10, 10);
			...
			scene->drawString("当前物体书：" + IntToString(scene->getVisibleObjectNumber()), 650, 10, EColor(255, 255, 0));
		...
		else
		{
			...
			scene->drawString("剩余的人 -" + IntToString(gameMgr->getLastEnemyNumber()), 20, 530, EColor(255, 255, 0));
