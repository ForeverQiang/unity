...
namespace E3D
{
	ESceneManager::ESceneManager() :mLightEnable(true), mVisibleObjectNumber(0), mVisiblePolyonNumber(0), mTotalPolyonNumber(0), mTerrainMesh(NULL)
	{
		mCamera = new ECamera();
	...
	void ESceneManager::update()
	{
		...
		//世界坐标系直接变换到屏幕坐标
		//世界坐标系->背面消除->摄像机坐标系变换->裁剪->光照计算->透视变换->屏幕变换
		...
		{
			Draw_RenderList4D_Solid(mRenderList);
		}
	}
	EMesh * ESceneManager::createTerrain(const EString & textureFileName, const EString & heightMapFileName, EFloat uTitle, EFloat vTitle, EFloat blockSize)
	{
		if (mTerrainMesh)
		{
		...
		if (!heigthMap->isValid() || heigthMap->getWidth() % 2 ==0 || heigthMap->getHeight() % 2 == 0)
		{
