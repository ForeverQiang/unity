	...
	EObject4D * LoadOgreMesh(const EString & meshName, const EVector4D & worldPos, const EVector4D & scale)
	{
		...
		EObject4D * head = NULL;
		{
			...
			while (!in.eof()&&(SUBMESH == M_SUBMESH))
			{
				...
				{
					//存储面顶点索引
					std::vector<EUShort> faceVertecIndices = ReadArray<EUShort>(in, indexCount);
					//读取模型顶点索引
					indexArray.assign(faceVertecIndices.begin(), faceVertecIndices.end());
				}
				...
				for (EInt i = 0; i < curObject->polyonNumber; i++)
				{
					...
					//这里将变换后的顶点指针交给每个多边形，是为了防止变换之后定点索引依旧是最初的顶点坐标
