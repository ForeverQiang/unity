...
namespace E3D
{
	...
	EGameWindow::EGameWindow(const EString & windowTitile, HINSTANCE hInstance) :mWindowTitle(windowTitile), mHinstance(hInstance), mCanvasListener(NULL), mQuite(false)
	{
	...
	bool EGameWindow::createWindow()
	{
		mHwnd = ::CreateWindow("E3D", mWindowTitle.c_str(),
			//窗口不可更改大小
			WS_OVERLAPPEDWINDOW &(WS_MAXIMIZEBOX)&(~WS_THICKFRAME), CW_USEDEFAULT, CW_USEDEFAULT, SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0, mHinstance, 0);
		if (!mHwnd)
		{
			::MessageBox(0, "Creae Window Failed", 0, 0);
			return false;
		}
