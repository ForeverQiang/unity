...
#define EPOLY_ATTR_SHADE_MODE_GOUTAUD        0x0010
...
#define EOBJECT_STATE_CULLED                 0x0004
	...
	struct CompEPolyonF4D
	{
		EBool operator()(const EPolyonF4D& polyA, const EPolyonF4D& polyB)
		{
			...
			if (Abs(zA-zB<0.005f))
			{
				zA = (polyA.transformList[0].z + polyA.transformList[1].z + polyA.transformList[2].z);
				zB = (polyB.transformList[0].z + polyB.transformList[1].z + polyB.transformList[2].z);
	...
	//对EObject进行变换，可以进行位移变换和旋转变换transformType指定了对哪一个定点列表进行变换，
	//transformBasis 指定了是否要对朝向进行变换
	void Transform_Object4D(EObject4D* object, const EMatrix44 & mat, OBJ_TRANSFORM_TYPE transformType/*, EBool transformBasis = true*/);
	...
	void Inert_Object4D_To_RenderList4D(ERenderList4D * renderList, EObject4D * object, OBJ_TRANSFORM_TYPE transformType = TRANSFORM_LOCAL_TO_TRANS, bool transformScaleAndPosition = false);
	...
	//局部坐标定编列表变换值世界坐标，这里直接使用加法，而美哟砽与矩阵相乘的方法，因为没有必要
	...
	//消除背面，背面消除算法的摄像机视点是基于世界坐标的，所以需要在顶点变换成员摄像机坐标钱进行背面消除
