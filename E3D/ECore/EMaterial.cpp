	...
	void ParseMaterialScript(const EString & scriptName)
	{
		...
		if (in.good())
		{
			...
			while (in.good())
			{
				...
				if (!info.empty())
				{
					//遇到“{"加1，遇到”}“减1，当brackers为0时，跳出当前材质
					EInt brackets = 0;
					...
					if (index == 0)
					{
						...
						while (brackets!=0)
						{
							...
							else
							{
								...
								if (ambientIndex == 0)
								{
									sscanf(info.c_str(), "%s%f%f%f", temp, &materail->ambient.r, &materail->ambient.g, &materail->ambient.b);
									continue;
								}
								...
								if (diffuseIndex == 0)
								{
									sscanf(info.c_str(), "%s%f%f%f", temp, &materail->diffuse.r, &materail->diffuse.g, &materail->diffuse.b);
									continue;
								}
								...
								if (specularIndex == 0)
								{
									sscanf(info.c_str(), "%s%f%f%f", temp, &materail->specular.r, &materail->specular.g, &materail->specular.b);
									continue;
								}
