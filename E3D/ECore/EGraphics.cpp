...
namespace E3D
{
	...
	{
		//材质脚本，首先获取默认路径
		hBitmap = (HBITMAP)::LoadImage(GHInstance, (LPCWSTR)GetPath(filename).c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	...
	void EGraphics::clearBuffer(const EColor & c)
	{
		...
		//重置深度缓存
		//注意，这里emset只能对int类型的数组进行初始化，所以直接使用了Int类型
	...
	void EGraphics::drawString(const EString &str, EInt x, EInt y, const EColor & c) 
	{
		...
		::TextOut(GBufferedHDC, x, y, (LPCWSTR)str.c_str(), str.length());
	...
	void EGraphics::setPixel(EInt x, EInt y, const EColor & c)
	{
		...
		//因此，在调用setPixel之前应先检测checkZ，返回true在调用setPixel
