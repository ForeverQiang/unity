...
namespace E3D
{
	EFrustum::EFrustum(CAMERA_TYPE mode, const EVector4D & pos, const EVector4D & dir,
		...
		EFloat viewportWidth, EFloat viewportHeight) :camMode(mode), position(pos), direction(dir), camTarget(target),
		...
		mPerspectiveToScreen(EMatrix44::IDENTITY) 
	{
		...
		EFloat tam_fov_div2 = tan(Degree_TO_Radian(fov * 0.5f));
		view_dist = 0.5f * viewplane_width / tam_fov_div2;
