...
namespace E3D
{
	...
	EVertex4D::EVertex4D(EFloat xf, EFloat yf, EFloat zf) : x(xf), y(yf), z(zf), w(1), u(0.0f), v(0.0f) {
	...
	EPolyonF4D::EPolyonF4D() : state(0), attribute(0), color(0xffffff)/*, per(NULL), next(NULL)*/ {
	...
	void Local_To_World_RenderList4D(ERenderList4D * renderList, const EVector4D & pos, OBJ_TRANSFORM_TYPE transformType)
	{
		...
		case TRANSFORM_TRANS:
		{
			for (ERenderList4D::Itr itr = renderList->polyData.begin(); itr != renderList->polyData.end(); ++itr)
			{
				...
				GetVertex4DAddVector4D(poly->transformList[0], pos, poly->localList[0]);
				GetVertex4DAddVector4D(poly->transformList[1], pos, poly->localList[1]);
				GetVertex4DAddVector4D(poly->transformList[2], pos, poly->localList[2]);
	...
	bool Remove_Backface(EPolyon4D * poly, EFrustum * camera)
	{
		...
		//这里使用变换后的多边形定点列表，因为背面消除只能在顶点被变换为世界坐标之后才能进行
		//而在这里一般在相机坐标系内对其进行背面消除
		//顶点是按顺时针方向排列的 u= p0->p1 v = p0->p2 n = uXv
	...
	bool Remove_Backface(EPolyonF4D * poly, EFrustum * camera)
	{
		...
		//而在这里一般在相机坐标系内对其进行背面消除
		//顶点时顺时针方向排列的u = p0-p1 v = p0-p2 m=uXv
	...
	void Light_polyonF4D(EPolyonF4D * poly, EFrustum * camera)
	{
		...
		//这里使用了EInt类型里面没有使用EColor类型，原因是后面要进行颜色的累加
		...
		for (LightItr itr = GLights->begin(); itr != GLights->end(); ++itr)
		{
			...
			else if (light->lightType == LIGHT_DIRECTION)
			{
			   ...
			   //这里当夹角小于90度，其点积大于零
				...
				if (dot > 0.0f)
				{
					...
					//接收光照的强度，多边形发现与光照方向的夹角越小，那么其接收强度越大
					//夹角越大，接收强度越小，多边形越暗
				...
				{
					//这里当多边形是北朝光源时，也进行一些处理，只有把它的颜色调得很暗，这样才比较协调
			...
			else if (light->lightType == LIGHT_POINT)
			{
				...
				{
					//这里当多边形是背面朝光源时，也进行了一些处理，只是把它的颜色调得很暗，这样才比较协调
					sumColorR += baseColor.r * light->shadowFactor;
					sumColorG += baseColor.g * light->shadowFactor;
					sumColorB += baseColor.b * light->shadowFactor;
	...
	void World_To_Screen_RenderList4D(ERenderList4D * renderList, EFrustum * camera)
	{
		...
		//在相机坐标系中对渲染列表的多变性进行深度排序
		//这里基于这样一个事实，即虽然x、y坐标发生了变化，但是z坐标系依旧是之前摄像机坐标系下的z值
		...
		while (obj)
		{
			for (EInt i = 0; i < obj->polyonNumber; i++)
			{
					...
					!(obj->polyonList[i].state & EPOLY_STATE_BACKFACE))
				{
					...
					EGraphics::drawLine(obj->transformList[index0].x, obj->transformList[index0].y, obj->transformList[index1].x, obj->transformList[index1].y, EColor(obj->polyonList[i].color));
					EGraphics::drawLine(obj->transformList[index1].x, obj->transformList[index1].y, obj->transformList[index2].x, obj->transformList[index2].y, EColor(obj->polyonList[i].color));
					EGraphics::drawLine(obj->transformList[index0].x, obj->transformList[index0].y, obj->transformList[index2].x, obj->transformList[index2].y, EColor(obj->polyonList[i].color));

				}
				obj->polyonList[i].state = EPOLY_STATE_ACTIVE;
				//绘制完毕，在这里恢复多边形，消除背面标记
			}
		...
		while (obj)
		{
			for (EInt i = 0; i < obj->polyonNumber; i++)
			{
					...
					!(obj->polyonList[i].state & EPOLY_STATE_BACKFACE))
				{
					...
					EGraphics::fillTriangle(obj->transformList[index0].x, obj->transformList[index0].y, obj->transformList[index1].x, obj->transformList[index1].y, obj->transformList[index2].x, obj->transformList[index2].y, EColor(obj->polyonList[i].color));
				}
	...
	void Draw_RenderList4D_Solid(ERenderList4D * renderList)
	{
		...
		for (ERenderList4D::Itr itr = renderList->polyData.begin(); itr != renderList->polyData.end() ; ++itr)
		{
			...
			{
				EGraphics::fillTriangle(poly->transformList[0].x, poly->transformList[0].y, poly->transformList[1].x, poly->transformList[1].y, poly->transformList[2].x, poly->transformList[2].y, EColor(poly->color));
			}
	...
	//绘制平底为下三角的三角形
	void Draw_Top_Trangle(EFloat x0, EFloat y0, EFloat z0, EFloat u0, EFloat v0,
		EFloat x1, EFloat y1, EFloat z1, EFloat u1, EFloat v1,
		EFloat x2, EFloat y2, EFloat z2, EFloat u2, EFloat v2, EMaterial* mat, const EColor& color)
	{
		...
		for (EFloat y = y0; y < y2; y++)
		{
			...
			for (EFloat x = begX; x < endX; x++)
			{
				if (x > - 1 && y < SCREEN_HEIGHT && EGraphics::checkZ(x,y,z))
				{
					const EColor tex = mat->getPixel(begU, begV);
					EGraphics::setPixel(x, y, tex);
				}
	...
	//绘制下三角
	void Draw_Botton_Trangle(EFloat x0, EFloat y0, EFloat z0, EFloat u0, EFloat v0,
		EFloat x1, EFloat y1, EFloat z1, EFloat u1, EFloat v1,
		EFloat x2, EFloat y2, EFloat z2, EFloat u2, EFloat v2,
		EMaterial* mat, const EColor &color)
	{
		if (y1 < 0 || y0 > SCREEN_HEIGHT - 1)
			return;
